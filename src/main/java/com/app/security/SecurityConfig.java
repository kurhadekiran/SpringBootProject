...
public class SecurityConfig {
	...
	public SecurityFilterChain authorizeRequests(HttpSecurity http) throws Exception
	{
		...
		and().
		authorizeRequests()
		//.antMatchers("/product/**","/users/signup","/users/signin",
				.antMatchers("/address/{userId}**","/product/**","/Customer/createorder","/transaction/**", "/order/**","/users/signup","/users/signin",
				"/v*/api-doc*/**","/swagger-ui/**").permitAll()
		// only required for JS clnts (react / angular) : for the pre flight requests
		.antMatchers(HttpMethod.OPTIONS).permitAll()
//		.antMatchers("/products/purchase").hasRole("CUSTOMER")
//		.antMatchers("/products/add").hasRole("SUPPLIER")
//		.antMatchers("/products/add").permitAll()
//		.antMatchers("/products").hasRole("SUPPLIER")
